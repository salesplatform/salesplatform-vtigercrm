...
{strip}
    ...
    <div id="filterContainer" class="filterElements filterConditionsDiv">
                    ...
                    <div class="contents">
                        <div class="conditionList">
                            {foreach item=CONDITION_INFO from=$ALL_CONDITION_CRITERIA['columns']}
                                {include file='sp_custom_reports/CustomReportFilterCondition.tpl'|@vtemplate_path:$QUALIFIED_MODULE RECORD_STRUCTURE=$RECORD_STRUCTURE CONDITION_INFO=$CONDITION_INFO MODULE=$MODULE}
                            {/foreach}
                            {if count($ALL_CONDITION_CRITERIA) eq 0}
                                {include file='sp_custom_reports/CustomReportFilterCondition.tpl'|@vtemplate_path:$QUALIFIED_MODULE RECORD_STRUCTURE=$RECORD_STRUCTURE MODULE=$MODULE CONDITION_INFO=array()}
                            {/if}
                        </div>
                        <div class="hide basic">
                            {include file='sp_custom_reports/CustomReportFilterCondition.tpl'|@vtemplate_path:$QUALIFIED_MODULE RECORD_STRUCTURE=$RECORD_STRUCTURE CONDITION_INFO=array() MODULE=$MODULE NOCHOSEN=true}
                        </div>
                        {if $CAN_ADD_FILTERS}
                             <div class="addCondition">
                                <button type="button" class="btn btn-default">
                                    <i class="fa fa-plus"></i> &nbsp;
                                    {vtranslate('LBL_ADD_CONDITION',$MODULE)}
                                </button>
                            </div>
                        {/if}
