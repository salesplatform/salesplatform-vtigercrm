...
class Vtiger_PackageUpdate extends Vtiger_PackageImport {
	...
	function update_CronTasks($modulenode) {
		...
		foreach ($modulenode->crons->cron as $importCronTask) {
            ...
            Vtiger_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description","$importCronTask->laststart","$importCronTask->lastend");
